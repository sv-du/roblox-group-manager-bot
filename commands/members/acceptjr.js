...
const acceptJoinRequestsCoolDown = new Set();
...
exports.run = async(message, client, args) => {
    ...
    if(acceptJoinRequestsCoolDown.has(message.author.id)) {
    ...
    let username = args[0];
    if(!username) {
        return message.channel.send(embedMaker('No Username Supplied', "You didn't supply a username for me to accept the join request of"));
    }
    ...
    try {
        await roblox.handleJoinRequest(configFile.groupID, rbxID, true);
    } catch (err) {
        return message.channel.send(embedMaker("Error", `There was an error while attmpting to accept ${username}'s join request: ${err}`));
    }

    message.channel.send(embedMaker("Success", `You have successfully accepted ${username}'s join request`));
    acceptJoinRequestsCoolDown.add(message.author.id);
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, rbxID, "AcceptJoinRequest", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(rbxID, "Accept Join Request", `Your join request in the group ${groupName} has been accepted by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        acceptJoinRequestsCoolDown.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**acceptjr <username>**";
    let description = "Accepts the join request of <username>";
