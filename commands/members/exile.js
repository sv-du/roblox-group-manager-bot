...
exports.run = async(message, client, args) => {
    ...
    if(exileCoolDowns.has(message.author.id)) {
    ...
    let canUserExileOthers = userPermissions.groupMembershipPermissions.removeMembers;

    if(canUserExileOthers == false) {
        return message.channel.send(embedMaker("No permission", `Your Roblox account doesn't have permission to exile members!`));
    }

    let username = args[0];
    if(!username) {
        return message.channel.send(embedMaker('No Username Supplied', "You didn't supply a username for me to promote"));
    }
    ...
    try {
        await roblox.exile(configFile.groupID, rbxID);
    } catch (err) {
        return message.channel.send(embedMaker("Error", `There was an error while attempting to exile ${username} from the group: ${err}`));
    }

    message.channel.send(embedMaker("Success", `You have successfully exiled ${username} from the group`));
    exileCoolDowns.add(message.author.id);
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, rbxID, "Exile", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(rbxID, "Exiled", `You have been exiled in the group ${groupName} by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        exileCoolDowns.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**exile <username>**";
    let description = "Exiles <username> from the group";
