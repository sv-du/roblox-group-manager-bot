...
class Log {
    ...
    constructor(authorID, authorRBXID, userID, amountOfRobux, type, caseNum) {
        this.authorID = authorID;
        this.authorRBXID = authorRBXID;
        this.userID = userID;
        this.amountOfRobux = amountOfRobux;
        this.type = type;
...
exports.run = async(message, client, args) => {
    ...
    if(configFile.groupPayoutsEnabled != true) {
        return message.channel.send(embedMaker("Not Enabled", `This command isn't enabled on the bot's configuration, to enable, please set the value of groupPayoutsEnabled in the configuration file to true`));
    }

    if(payCoolDowns.has(message.author.id)) {
    ...
    let canUserUseFunds = userPermissions.groupEconomyPermissions.spendGroupFunds;

    if(canUserUseFunds == false) {
        return message.channel.send(embedMaker("No permission", `Your Roblox account doesn't have permission to use group funds!`));
    }

    let username = args[0];
    if(!username) {
        return message.channel.send(embedMaker('No Username Supplied', "You didn't supply a username for me to pay"));
    }
    ...
    let robux = args[1];
    if(!robux) {
        return message.channel.send(embedMaker("No Robux Supplied", `You didn't supply the amount of robux that you wanted to pay to this user`));
    }

    if(isNaN(robux)) {
        return message.channel.send(embedMaker("Invalid Robux Amount", `The amount of robux that you supplied isn't valid cause it's not a number`));
    }

    try {
        await roblox.groupPayout(configFile.groupID, rbxID, robux);
    } catch (err) {
        return message.channel.send(embedMaker("Error", `There was an error while attempting to pay ${username} ${robux} robux: ${err}`));
    }

    message.channel.send(embedMaker("Success", `You have successfully payed ${username} ${robux} robux`));
    payCoolDowns.add(message.author.id);
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, rbxID, robux, "Payout", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(rbxID, "Payment", `You have been payed ${robux} robux in the group ${groupName} by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        payCoolDowns.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**pay <username> <amount of robux>**";
    let description = "Pays <username> the amount of robux that you supply";
