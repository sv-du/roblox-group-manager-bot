...
const promoteCoolDowns = new Set();
...
exports.run = async(message, client, args) => {
    ...
    if(promoteCoolDowns.has(message.author.id)) {
    ...
    try {
        await roblox.promote(configFile.groupID, rbxID);
    } catch (err) {
        return message.channel.send(embedMaker("Error", `There was an error while attempting to promote ${username}: ${err}`));
    }

    let newRankID = await roblox.getRankInGroup(configFile.groupID, rbxID);
    let newRankName = await roblox.getRankNameInGroup(configFile.groupID, rbxID);

    message.channel.send(embedMaker("Success", `You have successfully promoted ${username} from ${oldRankName} (${oldRankID}) to ${newRankName} (${newRankID})`));
    promoteCoolDowns.add(message.author.id);
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, rbxID, "Promote", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(rbxID, "Promotion", `You have been promoted in the group ${groupName} from ${oldRankName} to ${newRankName} by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        promoteCoolDowns.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**promote <username>**";
    let description = "Promotes <username> in the group";
