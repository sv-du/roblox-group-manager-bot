...
exports.run = async(message, client, args) => {
    ...
    if(demoteCoolDowns.has(message.author.id)) {
    ...
    let username = args[0];
    if(!username) {
        return message.channel.send(embedMaker('No Username Supplied', "You didn't supply a username for me to demote"));
    }
    let rbxID
    try {
    ...
    try {
        await roblox.demote(configFile.groupID, rbxID);
    } catch (err) {
        return message.channel.send(embedMaker("Error", `There was an error while attempting to demote ${username}: ${err}`));
    }
    ...
    message.channel.send(embedMaker("Success", `You have successfully demoted ${username} from ${oldRankName} (${oldRankID}) to ${newRankName} (${newRankID})`));
    demoteCoolDowns.add(message.author.id);
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, rbxID, "Demote", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(rbxID, "Demotion", `You have been demoted in the group ${groupName} from ${oldRankName} to ${newRankName} by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        demoteCoolDowns.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**demote <username>**";
    let description = "Demotes <username> in the group";
