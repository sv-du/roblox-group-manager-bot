...
async function getLowestRankInGroup(groupID) {
...
exports.run = async(message, client, args) => {
    ...
    if(fireCoolDowns.has(message.author.id)) {
    ...
    let canUserManageRoles = userPermissions.groupMembershipPermissions.changeRank;

    if(canUserManageRoles == false) {
        return message.channel.send(embedMaker("No permission", `Your Roblox account doesn't have permission to change user's ranks!`));
    }

    let username = args[0];
    if(!username) {
        return message.channel.send(embedMaker('No Username Supplied', "You didn't supply a username for me to fire"));
    }
    ...
    let newRankID = await getLowestRankInGroup(configFile.groupID);
    ...
    try {
        ...
        return message.channel.send(embedMaker("Error", `There was an error while attempting to fire ${username}: ${err}`));
    }

    message.channel.send(embedMaker("Success", `You have successfully fired ${username} from ${oldRankName} (${oldRankID})`));
    fireCoolDowns.add(message.author.id);
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, rbxID, oldRankID, "Fire", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(rbxID, "Termination", `You have been terminated in the group ${groupName} from ${oldRankName} (${oldRankID}) by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        fireCoolDowns.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**fire <username>**";
    let description = "Fires <username> in the group, basically just ranking them to the lowest rank";
