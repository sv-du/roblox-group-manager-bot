...
const createAllyRequestCoolDowns = new Set();
...
class Log {
    ...
    constructor(authorID, authorRBXID, requestedGroupID, type, caseNum) {
...
async function createAllyRequest(groupID) {
    let axios = require('axios').default;
    let token = await roblox.getGeneralToken();
...
exports.run = async(message, client, args) => {
    ...
    if(createAllyRequestCoolDowns.has(message.author.id)) {
    ...
    if(!groupID) {
        return message.channel.send(embedMaker("No Group ID Supplied", `You didn't supply a group id for me to create an ally request to or the group id that you supplied isn't a number`));
    }
    ...
    if(configFile.loggingEnabled == true) {
        ...
        if(!channel) {
            ...
            let newLogObject = new Log(message.author.id, authorID, groupID, "CreateAllyRequest", caseNumDatabase);
    ...
    if(configFile.messagingEnabled == true) {
        ...
        let didError = false;
        let r = await sendUser(userToMessage, "Ally Request", `Your group, ${requestedGroupName}, has just received an ally request from the group ${groupName} by ${await roblox.getUsernameFromId(authorID)} (${authorID})`);
    ...
    setTimeout(() => {
        createAllyRequestCoolDowns.delete(message.author.id);
    }, ms(timeString));
}

exports.help = async() => {
    let name = "**createallyrequest <group id>**";
    let description = "Creates an ally request for <group id>";
